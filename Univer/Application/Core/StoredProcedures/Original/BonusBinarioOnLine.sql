use ClubeVantagens
go
If Exists (Select 'Sp' From sysobjects Where id = object_id('BonusBinarioOnLine'))
   Drop Procedure BonusBinarioOnLine
Go

Create PROCEDURE [dbo].[BonusBinarioonline] 
...
BEGIN TRANSACTION
   ...
   Begin 
      ...
      Else
      Begin
         SET @dataInicio = CAST(CONVERT(VARCHAR(8),GETDATE(),112) + ' 00:00:00' as datetime2);
         SET @dataFim = CAST(CONVERT(VARCHAR(8),GETDATE(),112) + ' 23:59:59' as datetime2);
      End
   End
   Else
   Begin
   ...
   */
   ...
   Begin
      ...
      End
         ...
         SELECT 
            temp.idFranqueado UsuarioID, 
            0 ReferenciaID,  
            @dataInicio DataInicio,  
            @dataFim DataFim, 
            temp.pontosPernaEsquerda ValorPernaEsquerda, 
            temp.pontosPernaDireita ValorPernaDireita, 
            (ISNULL(AcumuladoEsquerda, 0) + temp.pontosAcumuladoPernaEsquerda) AcumuladoEsquerda, 
            (ISNULL(AcumuladoDireita, 0) + temp.pontosAcumuladoPernaDireita) AcumuladoDireita, 
            GETDATE() DataCriacao, 
            temp.pontosLiquidosPernaEsquerda ValorDiaPernaEsquerda,
            temp.pontosLiquidosPernaDireita ValorDiaPernaDireita,
            temp.qualificadoresquerdoid QualificadorEsquerdoID,
            temp.qualificadordireitoid QualificadorDireitoID
         FROM 
...
--exec BonusBinarioonline null, null, null
--drop table ##temp
--exec BonusBinarioonline '20161006','20161006',1
