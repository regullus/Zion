use UniverDev
go
If Exists (Select 'Sp' From sysobjects Where id = object_id('spI_TabuleiroLog'))
   Drop Procedure spI_TabuleiroLog
go

Create  Proc [dbo].[spI_TabuleiroLog]
    @UsuarioID int,
    @BoardID int
...
go
Grant Exec on spI_TabuleiroLog To public
go

--Exec spI_TabuleiroLog @UsuarioID = 2580, @BoardID = 1
