   ...
   Begin
     ...
     Select 
      ...
      where 
         ...
         -- Retirado a checagem de ativo, só temos unilevel neste modelo
		 -- usu.StatusID = 2 and --Ativos
         usu.id not in (Select IdAfiliado 
                        from Usuario.Afiliados (nolock)
                        where ID = @id And Nivel = @NivelPai) --Verifica se afiliado já foi incluido na tabela
     )
     Begin
