...
CREATE PROCEDURE [dbo].[sp_XS_AtualizaQualificacaoBinario] (@usuarioID INT)
AS
	...
	BEGIN TRY
		...
		begin
			...
			begin
				...
				begin
					update Usuario.Qualificacao set QualificadorDireitaID = @usuarioID, DataQualificacao = dbo.GetDateZion() where UsuarioID = @idPai
				end
				else
				begin
					insert into Usuario.Qualificacao (UsuarioID, QualificadorEsquerdaID, QualificadorDireitaID, DataQualificacao) 
					values (@idPai, null, @usuarioID, dbo.GetDateZion())
				end
			end	
		end
		else
		begin
			...
			begin
				if exists(select 1 from Usuario.Qualificacao where UsuarioID = @idPai)
				begin
					update Usuario.Qualificacao set QualificadorEsquerdaID = @usuarioID, DataQualificacao = dbo.GetDateZion() where UsuarioID = @idPai
				end
				else
				begin
					insert into Usuario.Qualificacao (UsuarioID, QualificadorEsquerdaID, QualificadorDireitaID, DataQualificacao) 
					values (@idPai, @usuarioID, null, dbo.GetDateZion())
				end
			end	
		end
