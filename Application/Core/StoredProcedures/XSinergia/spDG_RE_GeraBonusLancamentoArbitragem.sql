...
go
If Exists (Select 'Sp' From sysobjects Where id = object_id('spDG_RE_GeraBonusLancamentoArbitragem'))
   Drop Procedure spDG_RE_GeraBonusLancamentoArbitragem
go

Create PROCEDURE [dbo].[spDG_RE_GeraBonusLancamentoArbitragem]  
...
BEGIN  
  ...
  --      and DataLancamento = @data;  
      ...
      Select  
         ...
         1 as  StatusID,  
         @data as Data,  
         U.ValorBonus as Valor,  
         U.PedidoID as PedidoID,  
         null as CicloID,  
         null as RegraItemID,  
		    '[NA]: ' + CONVERT(varchar(10), U.NivelAssociacao) +  
         ' | [VM]: ' + CONVERT(varchar(10), U.ValorMig, 127)   +  
		 ' | [VP]: ' + CONVERT(varchar(10), U.ValorPac, 127)   +    
         ' | [TX]: ' + CONVERT(varchar(10), A.Percentual, 128) +
		 ' | [RR]: ' + U.BonusReduzido as Descricao,     
         0 as ValorCripto       
      From   
         #TUsuario    U  
         Inner join  #TArbitragem A ON U.NivelAssociacao = A.NivelAssociacao
   ...
   BEGIN CATCH  
      ...
      RAISERROR ('Erro na execucao de spDG_RE_GeraBonusLancamentoArbitragem: %d: %s', 16, 1, @error, @message) WITH SETERROR;  
   END CATCH  
END  

go
   Grant Exec on spDG_RE_GeraBonusLancamentoArbitragem To public
go 


--Exec spDG_RE_GeraBonusLancamentoArbitragem '20200309'

--Select  * from  Rede.Bonificacao  Where  CategoriaID = 21 and data = '2020-03-09 23:59:59.000'
--Select  * from  Financeiro.Lancamento  Where contaId = 2 and CategoriaID = 21
