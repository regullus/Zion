...
Begin
   ...
   --Select @ValorDiaPernaDireita  = COALESCE(SUM(p.ValorDiaPernaDireita) ,0),
	--       @ValorDiaPernaEsquerda = COALESCE(SUM(p.ValorDiaPernaEsquerda),0)
   --From  Rede.Posicao p
   --Where p.UsuarioID = @idUsuario

   --if (@ValorDiaPernaDireita < @ValorDiaPernaEsquerda)
   --   Select @ValorDiaPernaDireita
   --else
   --   Select @ValorDiaPernaEsquerda
