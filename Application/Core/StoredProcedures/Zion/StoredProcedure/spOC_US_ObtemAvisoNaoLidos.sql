Use ??????????
go
If Exists (Select 'Sp' From sysobjects Where id = object_id('spOC_US_ObtemAvisoNaoLidos'))
   Drop Procedure spOC_US_ObtemAvisoNaoLidos
go

Create PROCEDURE [dbo].[spOC_US_ObtemAvisoNaoLidos]
   @UsuarioID   int
...
BEGIN
   ...
   Declare
      @dataInicio   datetime2 = CAST(CONVERT(VARCHAR(8), dbo.GetDateZion()-1, 112) + ' 00:00:00' as datetime2),
      @dataFim      datetime2 = CAST(CONVERT(VARCHAR(8), dbo.GetDateZion()-1, 112) + ' 23:59:59' as datetime2),
...
go
   Grant Exec on spOC_US_ObtemAvisoNaoLidos To public
go

-- Exec spOC_US_ObtemAvisoNaoLidos 1000
