...
IF OBJECT_ID('dbo.sp_JB_AtualizaQualificacaoBinario', 'P') IS NOT NULL
	DROP PROCEDURE [dbo].[sp_JB_AtualizaQualificacaoBinario];
GO

CREATE PROCEDURE sp_JB_AtualizaQualificacaoBinario (@usuarioID INT)
AS
	...
	BEGIN TRY
		...
		begin
			...
			begin
				...
				begin
					insert into Usuario.Qualificacao values (@idPai, @idCiclo, null, @usuarioID, getdate(), 1)
				end
			end	
		end
		else
		begin
			...
			begin
				...
				begin
					insert into Usuario.Qualificacao values (@idPai, @idCiclo, @usuarioID, null, getdate(), 1)
				end
			end	
		end
	...
	BEGIN CATCH
		...
		RAISERROR ('Erro na execucao de sp_JB_AtualizaQualificacaoBinario: %d: %s', 16, 1, @error, @message) WITH SETERROR;
	END CATCH 