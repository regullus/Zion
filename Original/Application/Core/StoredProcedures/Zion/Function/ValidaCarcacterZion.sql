...
BEGIN
	BEGIN TRY
		...
		DECLARE 
		   @CategoriaID	INT = 13, -- Bônus de Indicação Direta
		   @RegraID		INT = 1,
		...
		FROM 
			...
			LEFT  JOIN Rede.Bonificacao          Bon     (NOLOCK) ON Bon.PedidoID = Ped.ID AND Bon.CategoriaID = @CategoriaID
			INNER JOIN Loja.ProdutoValor          Val     (NOLOCK) ON Val.ProdutoID = Prd.ID
			INNER JOIN Loja.PedidoPagamento       LPag    (NOLOCK) ON LPag.PedidoID = Ped.ID
			INNER JOIN Loja.PedidoPagamentoStatus LPagSta (NOLOCK) ON LPagSta.PedidoPagamentoID = LPag.ID
			INNER JOIN Rede.Ciclo                 RCic    (NOLOCK) ON LPagSta.Data BETWEEN RCic.DataInicial AND RCic.DataFinal
		WHERE 
		...
		FROM 
		   #TPedidos
		   INNER JOIN Rede.Upline_Ciclo Upl (NOLOCK) ON Upl.UsuarioID = #TPedidos.UsuarioID AND Upl.CicloID = #TPedidos.CicloID
		   LEFT  JOIN Usuario.AtivacaoMensal AtvMes (NOLOCK)  ON AtvMes.UsuarioID = Upl.Upline AND AtvMes.CicloID = #TPedidos.CicloID
		WHERE 
		...
		BEGIN
			...
			BEGIN
				...
				FROM 
					...
					INNER JOIN #TUpline Upl  ON Upl.UsuarioID = T.UsuarioID
				WHERE 
